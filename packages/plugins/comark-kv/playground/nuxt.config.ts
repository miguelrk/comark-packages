...
export default defineNuxtConfig({
  ...
  // pnpm link: in playground/package.json instead.
  alias: kvAliases,
  runtimeConfig: {
    public: {
  ...
  app: {
    baseURL: process.env.NODE_ENV === 'development' ? '/' : '/comark-kv/',
    head: {
      htmlAttrs: { lang: 'en' },
  ...
  nitro: {
    ...
    prerender: {
      crawlLinks: true,
      routes: ['/'],
      ignore: ['/LICENSE', '/comark-kv/LICENSE'],
    },
  },
  vite: {
    resolve: {
    ...
    server: {
      fs: {
        allow: [
          root,
          fileURLToPath(new URL('../../comark', import.meta.url)),
        ],
      },
    },
    optimizeDeps: {
